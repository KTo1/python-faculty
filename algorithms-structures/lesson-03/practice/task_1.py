...
Получение элементов списка быстрее, полагаю потому, что список адресут память напрямую
...
ситуация в корне меняется если нужно удалить произовальный элемент из списка, на это времени уходит намного больше,
думаю потому, что нужно перераспределять памямть под оставшуюся часть списка, видимо поэтому сложность алгоритма O(n).
"""
