...
Предлагаемый вариант решения 4, самый быстрый, по причинам описанным выше
Вариант 5 одной строкой, можно ускорить, если принт убрать)   
...
def func_5():
    dct = {array.count(i): i for i in array}
    return f'Чаще всего встречается число {dct[max(dct)]}, ' \
           f'оно появилось в массиве {max(dct)} раз(а)'
